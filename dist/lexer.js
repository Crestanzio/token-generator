...
function lexer(input, ArrayOfTokens) {
    const tokens = [];
    const checked = [];
    function isNotExist(char) {
        let counter = 0;
    ...
    for (let cursor = 0, length = input.length; cursor < length;) {
        ...
        Tokens: for (let token = 0, length = ArrayOfTokens.length; token < length; token++) {
            let currentType = ArrayOfTokens[token].type, currentValue = ArrayOfTokens[token].value, currentLength = ArrayOfTokens[token].length;
            if (isNotExist(character))
                throw new Error(`Unexpected token '${character}'`);
            if (typeof currentValue === "string") {
                let FCharMatch = currentValue.startsWith(character);
                if (FCharMatch && currentLength) {
                    let expression = "", counter = 0;
                    switch (true) {
                    ...
                    let currentValueLength = currentValue.length, LCharIndex = currentValueLength - 1;
                    for (let index = 0; index < currentValueLength; index++) {
                        let lastIndex = index === LCharIndex, LCharMatch = currentValue.charAt(LCharIndex) === character;
                        if (isNotExist(character))
                            throw new Error(`Unexpected token '${character}'`);
                        switch (true) {
